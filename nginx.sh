...
System_Lib() {
    if [ "${PM}" == "yum" ] || [ "${PM}" == "dnf" ]; then
        Pack="git perl-IPC-Cmd gcc gcc-c++ curl curl-devel libtermcap-devel ncurses-devel libevent-devel readline-devel libuuid-devel"
        ${PM} install ${Pack} -y
...
Download_Src() {
    ...
    elif [ "${version}" == "nginx-quic" ]; then
        wget -O ${Setup_Path}/src.tar.gz https://hg.nginx.org/nginx-quic/archive/tip.tar.gz
        tar -xvf src.tar.gz 
        mv nginx-quic-* src
    else
    ...
    if [ "${version}" == "nginx-quic" ]; then
        wget --no-check-certificate https://github.com/quictls/openssl/archive/refs/tags/openssl-3.0.7+quic1.tar.gz
        tar -xvf openssl-3.0.7+quic1.tar.gz
        mv openssl-openssl-3.0.7-quic1 openssl
        rm -f openssl-3.0.7+quic1.tar.gz
    else
        curl -k -O https://www.openssl.org/source/old/3.0/openssl-3.0.0-alpha17.tar.gz
        tar -xvf openssl-3.0.0-alpha17.tar.gz
        mv openssl-3.0.0-alpha17 openssl
        rm -f openssl-3.0.0-alpha17.tar.gz
    fi
...
Install_Configure() {
    ...
    if [ "${version}" == "nginx-quic" ]; then
        ENABLE_HTTP3="--with-http_v3_module --with-stream_quic_module"
        CONFIGURE_PATH="./auto/configure"
    else
    ...
    ${CONFIGURE_PATH} --user=www --group=www --prefix=${Setup_Path} ${ENABLE_LUA} --add-module=${Setup_Path}/src/ngx_cache_purge ${ENABLE_STICKY} --with-openssl=${Setup_Path}/src/openssl --with-pcre=pcre-${pcre_version} ${ENABLE_HTTP2} ${ENABLE_HTTP3} --with-http_stub_status_module --with-http_ssl_module --with-http_image_filter_module --with-http_gzip_static_module --with-http_gunzip_module --with-ipv6 --with-http_sub_module --with-http_flv_module --with-http_addition_module --with-http_realip_module --with-http_mp4_module --with-ld-opt="-Wl,-E" --with-cc-opt="-Wno-error -Wno-deprecated-declarations" ${jemallocLD} ${ENABLE_WEBDAV} ${ENABLE_NGX_PAGESPEED} ${ADD_EXTENSION} ${i_make_args}
...
Set_Version() {
    if [ "${version}" == "tengine" ]; then
        ...
        echo "nginx-quic" >${Setup_Path}/version.pl
        echo "nginx-quic" >${Setup_Path}/version_check.pl
    else
        echo "${nginxVersion}" >${Setup_Path}/version.pl
    fi
